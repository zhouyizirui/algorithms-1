...
function VigenèreCipher(key, abc) {
    ...
    this.encode = function (str) {
        ...
        for(var i = 0;i < str.length;i++){
            encodeStr += this.getKeyArr(this.keys[i%this.keys.length])[this.alphabet.indexOf(str.charAt(i))] || str.charAt(i);
        }
        return encodeStr;
        //...
    };
    this.decode = function (str) {
        var decodeStr = '';
        for(var i = 0;i < str.length;i++){
            decodeStr += this.alphabet[this.getKeyArr(this.keys[i%this.keys.length]).indexOf(str.charAt(i))] || str.charAt(i);
        }
        return decodeStr;
        //...
    };
}
VigenèreCipher.prototype.getKeyArr = function(k){
    var which = this.alphabet.indexOf(k);
    return this.alphabet.slice(which)+this.alphabet.slice(0,which);
}
